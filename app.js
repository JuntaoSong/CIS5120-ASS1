...
const updatePage = async () => {
  ...
  const lowCalorieFruits = fruitsArray.filter(fruit => fruit.nutritions.calories < 50);
  // TODO: Create a new HTML element to display your data
  const fruitElements = lowCalorieFruits.map(fruit => {
    const fruitElement = document.createElement('div');
    fruitElement.textContent = `Name: ${fruit.name}, Calories: ${fruit.nutritions.calories}`;
    return fruitElement;
  });
  // TODO: Append your new element to the page
  fruitElements.forEach(fruitElement => gallery.appendChild(fruitElement));
};
